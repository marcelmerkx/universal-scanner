#!/usr/bin/env python3
"""
Download Barcode Images from CSV for Training Data

This script downloads barcode images from URLs listed in a CSV file and saves them
with filename-safe normalized names in the training data structure.

Input: barcodes.csv with columns "url" and "reference"
...
The input file is generated using this SQL query:
        ...
        WHERE
            scan_code not like '___U_______'  -- Exclude container codes (4th char != U)
...
The script:
1. Reads CSV file with barcode image URLs and references
...
# Default paths
DEFAULT_CSV_FILE = "detection/training_data/00_raw/barcodes.csv"
DEFAULT_OUTPUT_DIR = Path("detection/training_data/00_raw/code_qr")
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            request = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; BarcodeDataCollector/1.0)"
                }
            )
...
def process_csv_file(csv_file: Path, output_dir: Path, limit: Optional[int], logger: logging.Logger, timeout: int = REQUEST_TIMEOUT):
    """
    Process CSV file and download barcode images.
    ...
    if stats["downloaded"] > 0:
        logger.info(f"\n✓ Successfully downloaded {stats['downloaded']} barcode images!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download barcode images from CSV file",
    ...
    # Start processing
    logger.info("Barcode Image Downloader Starting...")
