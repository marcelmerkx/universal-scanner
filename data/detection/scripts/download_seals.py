#!/usr/bin/env python3
"""
Download Seal Images from CSV for Training Data

This script downloads seal images from URLs listed in a CSV file and saves them
with filename-safe normalized names in the training data structure.

Input: seals.csv with columns "url" and "reference"
...
Usage Examples:
    # Download seals (default)
    python3 detection/scripts/download_seals.py
    
    # Download with custom settings
    python3 detection/scripts/download_seals.py \\
        --csv-file path/to/custom.csv \\
        --output-dir detection/training_data/00_raw/code_seal \\
        --limit 100
...
The script:
1. Reads CSV file with seal image URLs and references
...
# Default paths
DEFAULT_CSV_FILE = "detection/training_data/00_raw/seals.csv"
DEFAULT_OUTPUT_DIR = Path("detection/training_data/00_raw/code_seal")
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            request = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; SealDataCollector/1.0)"
                }
            )
...
def process_csv_file(csv_file: Path, output_dir: Path, limit: Optional[int], logger: logging.Logger, timeout: int = REQUEST_TIMEOUT):
    """
    Process CSV file and download seal images.
    ...
    if stats["downloaded"] > 0:
        logger.info(f"\n✓ Successfully downloaded {stats['downloaded']} seal images!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download seal images from CSV file",
    ...
    parser.add_argument(
        ...
        help="Path to CSV file with seal URLs and references"
    )
    ...
    # Start processing
    logger.info("Seal Image Downloader Starting...")
